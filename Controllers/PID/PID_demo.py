from scipy.spatial import KDTree  # KDTree是一种数据结构，用于快速查找多维空间中最接近某个点的数据点
...
def cal_target_index(robot_state, refer_path):
    ...
    for xy in refer_path:
        dis = np.linalg.norm(robot_state - xy)  # 计算向量的范数（即欧几里德距离）
        dists.append(dis)
...
def main():
    ...
    animation = camera.animate()  # 根据记录的图像生成动画
