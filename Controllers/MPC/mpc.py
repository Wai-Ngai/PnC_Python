...
class MyReferencePath:
    ...
    def calc_ref_trajectory(self, robot_state, dl=1.0):
        ...
        for i in range(T + 1):
            travel += abs(robot_state[3]) * dt    # s = v*t
...
def linear_mpc_control(xref, x0, delta_ref, ugv):
    ...
    constraints = []
    for t in range(T):
        cost += cvxpy.quad_form(u[:, t] - delta_ref[:, t], R)  # uTRu
        if t != 0:
            cost += cvxpy.quad_form(x[:, t] - xref[:, t], Q)  # xTQx

        A, B, C = ugv.state_space(delta_ref[1, t], xref[2, t])
        constraints += [
            x[:, t + 1] - xref[:, t + 1] == A @ (x[:, t] - xref[:, t]) + B @ (u[:, t] - delta_ref[:, t])]  # 运动学模型约束

    cost += cvxpy.quad_form(x[:, T] - xref[:, T], Qf)  # 终端状态代价

    constraints += [(x[:, 0]) == x0]  # 初始状态约束
    constraints += [cvxpy.abs(u[0, :]) <= MAX_VEL]  # 控制量阈值约束
    constraints += [cvxpy.abs(u[1, :]) <= MAX_STEER]

    # 创建一个凸优化问题，输入：问题的目标（代价最小），约束
    prob = cvxpy.Problem(cvxpy.Minimize(cost), constraints)
    prob.solve(solver=cvxpy.ECOS, verbose=False)  # 使用 ECOS 求解器进行求解，不输出过程信息
